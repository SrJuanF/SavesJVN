      ...
      <div className="absolute inset-0 flex items-center justify-center p-4">
        <div className="w-full max-w-lg rounded-2xl border border-white/10 bg-gradient-to-br from-white/10 to-white/5 shadow-2xl flex flex-col max-h-[85vh] overflow-hidden">
          <div className="flex items-center justify-between p-4 border-b border-white/10">
            <h3 className="text-lg font-bold">Purchase with QR</h3>
            <button className="rounded-md px-2 py-1 text-sm border border-white/20 hover:bg-white/10" onClick={() => !processing && onClose()}>Close</button>
          </div>
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
              <div className="flex items-center justify-between gap-3">
                <p className="text-sm font-semibold">Payment QR - Domestic merchant</p>
              ...
              <div className="mt-3">
                <p className="text-sm font-semibold">Micro-contribution</p>
                <div className="mt-2 grid grid-cols-1 md:grid-cols-2 gap-3">
                  <div>
                  <label className="text-sm">Select fund</label>
                  <select
                  ...
                  >
                    {funds.map((f) => (
                      <option key={String(f.id)} value={String(f.id)}>{`Fund #${String(f.id)}`}</option>
                    ))}
                  </select>
                  </div>
                  <div>
                  <label className="text-sm">Contribution method</label>
                  <select
                    value={method}
                    onChange={(e) => setMethod(e.target.value as any)}
                    className="mt-1 w-full rounded-md bg-black/30 border border-white/10 text-white focus:outline-none focus:ring-2 focus:ring-primary p-2"
                  >
                    <option value="round">Round up to nearest whole (0.01)</option>
                    <option value="fixed">Fixed amount (0.02)</option>
                    <option value="percent">Percentage of the transaction (0.02)</option>
                  </select>
                </div>
                </div>
              </div>
